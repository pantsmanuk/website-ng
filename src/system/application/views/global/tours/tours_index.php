...
foreach($tour_array as $row){
	...
	echo '<tr>';	
		if(is_array(@getimagesize($image_url))){ echo '<td><a href="'.$base_url.'tours/details/'.$row->id.'"><img src="'.$image_url.'" alt="'.$row->rank_name.'" /></a></td>'; }else{ echo '<td>&nbsp;</td>'; }
		echo '<td '.$bgstyle.'>';
...
- You may fly each tour as many times as you wish, but the reward will only be issued the first time.<br />
- You must complete one tour before starting another.<br />
- Flight hours from tours will be added to your total time at Euroharmony. Report the legs using the Pirep, or the flight logger.<br />
- After completing a tour, you should automatically receive your reward.
- You may set the weather (if flying offline), time and date as you wish. The tours are to be flown VFR with GA planes (except Capitals tour), so it's recommended to fly at daytime.<br />
- Make sure you review all individual requirements for each tour (hours needed, plane type etc). We can't accept tours that are not compatible with the regulations.<br />
- Make full stop landings at all airports of a tour. Touch-and-gos or flybys are not sufficient.<br />
- Respect all other restrictions regarding EuroHarmony flights, as they are presented in the Operations Manual.
