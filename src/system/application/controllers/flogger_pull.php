	...
	function flights($type = 'assigned')
	{
		...
		if($type == 'missions'){
			...
			if($num_rows > 0){
				...
				echo '</header>'."\n";
				echo '<data>'."\n";
				echo '<flight-list timestamp="'.$gmt_mysql_datetime.'">'."\n";
			...
			*/
			...
			|| $post_timestamp == '0000-00-00 00:00:00'){
				...
				foreach($list as $flight){
					...
					//combine days
					if($flight->mon == '1'){ $days .= '1'; } else{ $days .= '-'; }
					if($flight->tue == '1'){ $days .= '2'; } else{ $days .= '-'; }
					if($flight->wed == '1'){ $days .= '3'; } else{ $days .= '-'; }
					if($flight->mon == '1'){ $days .= '4'; } else{ $days .= '-'; }
					if($flight->mon == '1'){ $days .= '5'; } else{ $days .= '-'; }
					if($flight->mon == '1'){ $days .= '6'; } else{ $days .= '-'; }
					if($flight->mon == '1'){ $days .= '7'; } else{ $days .= '-'; }
