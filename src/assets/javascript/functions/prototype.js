...
'constructor'];
  ...
  function Str(key, holder, stack) {
    ...
    if (Type(value) === OBJECT_TYPE && typeof value.toJSON === 
'function') {
    ...
    switch (type) {
      ...
      case 'object':
        ...
        for (var i = 0, length = stack.length; i < length; i++) {
          if (stack[i] === value) {
            throw new TypeError("Cyclic reference to '" + value + "' in 
object");
          }
        }
        stack.push(value);
...
(function() {
  var arrayProto = Array.prototype,
      ...
      _each = arrayProto.forEach; // use native browser JS 1.6 
    ...
    if (receiver = $(receiver)) {
      if (options.insertion) {
        if (Object.isString(options.insertion)) {
          var insertion = { }; insertion[options.insertion] = 
responseText;
...
1]
  ...
  function replace_IE(element, content) {
    ...
    content = Object.toHTML(content);
    var parent = element.parentNode, tagName = 
parent.tagName.toUpperCase();
    ...
    if (tagName in INSERTION_TRANSLATIONS.tags) {
      ...
      var iterator;
      if (nextSibling)
        iterator = function(node) { parent.insertBefore(node, 
nextSibling) };
      else
  ...
  function isContent(content) {
    ...
    if (Object.isString(content) || Object.isNumber(content)) return 
true;
  ...
  function insertContentAt(element, content, position) {
    ...
    var childNodes = getContentFromAnonymousElement(tagName, 
content.stripScripts());
...
!/\S/.test(node.nodeValue))
  ...
  function _recursivelyFind(element, property, expression, index) {
    element = $(element), expression = expression || 0, index = index || 
0;
...
0;
  ...
  function previous(element, expression, index) {
    return _recursivelyFind(element, 'previousSibling', expression, 
index);
  }
...
' +
   'accessKey tabIndex encType maxLength readOnly longDesc 
frameBorder');
...
Element.Methods.ByTag[t.toUpperCase()]);
  ...
  if (F.SpecificElementExtensions) {
    extend = HTMLOBJECTELEMENT_PROTOTYPE_BUGGY ? extend_IE8 : 
Prototype.K;
  }
...
(function() {
  ...
  function getPixelValue(value, property, context) {
    ...
    var isPercentage = value.include('%'), isViewport = (context === 
document.viewport);

    if (/\d/.test(value) && element && element.runtimeStyle && 
!(isPercentage && isViewport)) {
      var style = element.style.left, rStyle = 
...
property.include('bottom') ||
  ...
  Element.Layout = Class.create(Hash, {
    ...
    _begin: function() {
      ...
      var position = getRawStyle(element, 'position'), width = 
element.offsetWidth;
...
this.element));
    ...
    _isPrepared: function() {
      return this.element.retrieve('prototype_element_layout_prepared', 
false);
    },

    _setPrepared: function(bool) {
      return this.element.store('prototype_element_layout_prepared', 
bool);
    },
  ...
  function clonePosition(element, source, options) {
    ...
    if (options.setLeft || options.setTop) {
      ...
      if (Element.getStyle(element, 'position') === 'absolute') {
        ...
        if (parent !== document.body) delta = 
Element.viewportOffset(parent);
      }
    }
...
defined.');
  ...
  function find(elements, expression, index) {
    index = index || 0;
    var match = Prototype.Selector.match, length = elements.length, 
matchIndex = 0, i;
...
=== 1 && (parts[0] === "~" || parts[0] === "+") && context.parentNode ? 
...
true || checkSet[i].nodeType === 1 && Sizzle.contains(context, 
checkSet[i])) ) {
					results.push( set[i] );
				}
			}
...
type ], "" );
	...
	if ( !set ) {
		set = typeof context.getElementsByTagName !== 
"undefined" ?
...
match, curLoop, inplace, result, not, isXMLFilter );
...
true );
		...
		"": function(checkSet, part, isXML){
			...
			if ( typeof part === "string" && !rNonWord.test( 
part ) ) {
				part = part.toLowerCase();
				nodeCheck = part;
...
+ " ";
			...
			for ( var i = 0, elem; (elem = curLoop[i]) != 
null; i++ ) {
				if ( elem ) {
...
elem.innerText || Sizzle.getText([ elem ]) || "").indexOf(match[3]) >= 
0;

			} else if ( name === "not" ) {
				var not = match[3];

				for ( var j = 0, l = not.length; j < l; 
j++ ) {
					if ( not[j] === elem ) {
						return false;
					}
...
!relativeHierarchySelector || hasParent ) {
							return 
makeArray( context.querySelectorAll( "[id='" + nid + "'] " + query ), 
extra );
						}
...
Form.Element.Serializers = (function() {
  ...
  function select(element, value) {
    if (Object.isUndefined(value))
      return (element.type === 'select-one' ? selectOne : 
selectMany)(element);
...
code);
  ...
  function _isButtonForWebKit(event, code) {
    switch (code) {
      ...
      case 1: return event.which == 2 || (event.which == 1 && 
event.metaKey);
  ...
  function _element(event) {
    ...
    if (currentTarget && currentTarget.tagName) {
      if (type === 'load' || type === 'error' ||
        (type === 'click' && currentTarget.tagName.toLowerCase() === 
'input'
...
document.createEvent('HTMLEvents').__proto__;
  ...
  function register(element, eventName, handler) {
    ...
    var uid = getUniqueElementID(element);
    var responder = GLOBAL.Event._createResponder(uid, eventName, 
handler);
    var entry = {
      responder: responder,
      handler:   handler
...
var $continue = new Error('"throw $continue" is deprecated, use "return" 
instead');
