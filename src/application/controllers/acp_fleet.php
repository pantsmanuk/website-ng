	...
	function edit($aircraft_id = NULL) {
		...
		elseif ($is_admin != '1') {
			...
			if ($valid == 'true' && $validation == 1) {
				...
				if ($to_rwy_length_min == '') {
					$to_rwy_length_min = NULL;
				}
				if ($to_rwy_length_max == '') {
					$to_rwy_length_max = NULL;
				}
				if ($land_rwy_length == '') {
					$land_rwy_length = NULL;
				}
				if ($v_rotate == '') {
					$v_rotate = NULL;
				}
				...
				if ($length != '' && is_numeric($length)) {
					$length_in = ($length * 100);
				} else {
					$length_in = $length;
				}
				if ($wingspan != '' && is_numeric($wingspan)) {
					$wingspan_in = ($wingspan * 100);
				} else {
				...
				if ($gross_weight != '' && is_numeric($gross_weight)) {
					$gross_weight_in = ($gross_weight * 100);
				} else {
					$gross_weight_in = $gross_weight;
				}
				if ($price != '' && is_numeric($price)) {
					$price_in = ($price * 1000000);
				} else {
					$price_in = $price;
				}

				if ($maximum_climb_rate != '' && is_numeric($maximum_climb_rate)) {
					$maximum_climb_rate_in = $maximum_climb_rate;
				} else {
					$maximum_climb_rate_in = '3600';
				}
				if ($maximum_desc_rate != '' && is_numeric($maximum_desc_rate)) {
					$maximum_desc_rate_in = $maximum_desc_rate;
				} else {
					$maximum_desc_rate_in = '3000';
				}
				...
				if ($aircraft_id > 0) {
					...
					if ($result['0']['length'] > 0) {
						$length = ($result['0']['length'] / 100);
					} else {
						$length = '';
					}
					if ($result['0']['wingspan'] > 0) {
						$wingspan = ($result['0']['wingspan'] / 100);
					} else {
						$wingspan = '';
					}
					if ($result['0']['height'] > 0) {
						$height = ($result['0']['height'] / 100);
					} else {
						$height = '';
					}
					...
					if ($result['0']['gross_weight'] > 0) {
						$gross_weight = ($result['0']['gross_weight'] / 100);
					} else {
						$gross_weight = '';
					}
					$crew = $result['0']['crew'];
					if ($result['0']['price'] > 0) {
						$price = ($result['0']['price'] / 1000000);
					} else {
						$price = '';
					}
