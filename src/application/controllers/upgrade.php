...
class Upgrade extends CI_Controller {
	...
	function testpirep() {
		...
		$query = $this->db->query("	SELECT 	pirep.id as id,
											...
											OR 		(pirep.submitdate = '" . $data['gmt_mysql_datetime'] . "'
		...
		if ($pilot_rows > 0
			...
			if ($post_propilot == '1') {
				...
				$pilot_data['pp_lastflight'] = $data['gmt_mysql_datetime'];
			...
			if ($tour_id != '') {
				$tour_award_return = $this->Pirep_model->tour_award($pilot_id, $tour_id, $award_id, $data['gmt_mysql_datetime']);
			}
			...
			if ($event_id != '' && $award_id != '') {
				$event_award_return = $this->Pirep_model->event_award($pilot_id, $event_id, $award_id, $data['gmt_mysql_datetime']);
			}
	...
	function test_tour_award() {
		...
		//call function
		$return_data = $this->Pirep_model->tour_award($pilot_id, $tour_id, $award_id, $data['gmt_mysql_datetime']);
