	...
	function edit($mission_id = NULL)
	{
		...
		elseif($acp_check_time != '' && strtotime($acp_check_time) >= $timeout_time && $is_admin == '1'){
			...
			if($valid == 'true' && $validation == 1){
				...
				if($dep_time == '' || $dep_time == '00:00:00'){ $dep_time = NULL; }
				else{ $dep_time = $dep_time.':00'; }
				if($arr_time == '' || $arr_time == '00:00:00'){ $arr_time = NULL; }
				else{ $arr_time = $arr_time.':00'; }
				if($dep_weather == ''){ $dep_weather = NULL; }
				if($arr_weather == ''){ $arr_weather = NULL; }
