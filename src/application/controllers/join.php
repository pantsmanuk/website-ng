...
class Join extends CI_Controller {
	...
	function index() {
		...
		//selected hubs
		if ($hub_1 == $hub_2 || $hub_1 == $hub_3 || $hub_2 == $hub_3) {
			$additional = '0';
		}
...
Good luck with your virtual career and welcome to the VA! ");
			...
			if ($valid == '1') {
				...
				if ($validation == 0) {
					$data['error'] .= '<li>Not all required fields have been completed</li>';
				}
				if ($email != $email2) {
					$data['error'] .= '<li>The email addresses do not match</li>';
				}
				if ($verify_mail == '0') {
					$data['error'] .= '<li>The email address is not valid</li>';
				}
				if ($dob_valid == '0') {
					$data['error'] .= '<li>The date of birth is not a valid date</li>';
				}
				if ($hub_1 == $hub_2 || $hub_1 == $hub_3 || $hub_2 == $hub_3) {
					$data['error'] .= '<li>Selected hubs are not unique</li>';
				}
				if ($time_sequence == '0') {
					$data['error'] .= '<li>There is an error in the flight times</li>';
				}
				if ($assign_id == '0') {
					$data['error'] .= '<li>Unable to assign a username. Please contact an administrator.</li>';
				}
